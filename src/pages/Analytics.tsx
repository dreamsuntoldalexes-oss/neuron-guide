          ...
          <div className="space-y-2">
            {leaders.map((tool, i) => (
              <Link key={tool.id} to={`/tools/${tool.id}`} className="flex items-center gap-3 p-2 rounded-xl hover:bg-muted/40 transition">
                ...
                <img src={tool.logo} alt={tool.name} width={36} height={36} loading="lazy" decoding="async" className="w-9 h-9 rounded-lg bg-muted" onError={(e) => { (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=${encodeURIComponent(tool.name)}&background=10b981&color=fff&size=64`; }} />
