...
export default function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem("ai-tools-user", JSON.stringify({ email, name }));
    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-6 relative overflow-hidden">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground mt-2">Join the AI tools community</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="glass-card p-6 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input type={showPass ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••"
                className="w-full bg-muted/50 border border-border rounded-xl py-2.5 pl-10 pr-10 text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-1 focus:ring-primary/50 transition" required />
          ...
          <button type="submit" disabled={loading} className="w-full py-3 rounded-xl font-heading font-semibold text-primary-foreground gradient-primary hover:opacity-90 transition active:scale-[0.98] disabled:opacity-60 flex items-center justify-center gap-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button type="button" onClick={() => handleOAuth("google")} className="flex items-center justify-center gap-2 py-2.5 rounded-xl border border-border bg-muted/30 text-foreground text-sm hover:bg-muted/60 transition">
            ...
            <button type="button" onClick={() => handleOAuth("apple")} className="flex items-center justify-center gap-2 py-2.5 rounded-xl border border-border bg-muted/30 text-foreground text-sm hover:bg-muted/60 transition">
